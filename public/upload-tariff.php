...
if ($_SERVER['REQUEST_METHOD'] === 'POST') {
    ...
    if (empty($errors)) {
        try {
            ...
            // Crear tarifa básica
            $jsonData = [
                ['level' => 'chapter', 'id' => '1', 'name' => 'Capítulo 1', 'amount' => '0.00'],
                ['level' => 'item', 'id' => '1.1', 'name' => 'Partida ejemplo', 'amount' => '0.00', 
                 'description' => 'Descripción ejemplo', 'unit' => 'ud', 'quantity' => '0.00', 
                 'iva_percentage' => '21.00', 'pvp' => '10.00']
            ];
            
            $stmt = $pdo->prepare("INSERT INTO tariffs (name, json_data) VALUES (?, ?)");
            $stmt->execute([$tariffName, json_encode($jsonData)]);
            $tariffId = $pdo->lastInsertId();
            
            $stmt = $pdo->prepare("
                INSERT INTO company_config 
                (tariff_id, name, nif, address, contact, logo_url, primary_color, secondary_color) 
                VALUES (?, ?, ?, ?, ?, ?, ?, ?)
            ");
            $stmt->execute([
                $tariffId,
    ...
    <div class="form-container">
        ...
        <?php if ($success): ?>
            <div class="alert alert-success">
                Tarifa creada correctamente. <a href="tariffs.php">Ver tarifas</a>
            </div>
        <?php endif; ?>
        ...
        <form method="POST">
            <div class="form-group">
                ...
                <input type="text" id="tariff_name" name="tariff_name" required 
                       value="<?= htmlspecialchars($_POST['tariff_name'] ?? '') ?>">
            </div>
            ...
            <div class="form-row">
                <div class="form-group">
                    ...
                    <input type="text" id="company_nif" name="company_nif" 
                           value="<?= htmlspecialchars($_POST['company_nif'] ?? '') ?>">
                </div>
                <div class="form-group">
            ...
            <div class="form-group">
                ...
                <textarea id="company_address" name="company_address"><?= htmlspecialchars($_POST['company_address'] ?? '') ?></textarea>
            </div>
            
            <div class="form-group">
                <label for="company_contact">Contacto:</label>
                <input type="text" id="company_contact" name="company_contact" 
                       value="<?= htmlspecialchars($_POST['company_contact'] ?? '') ?>">
            </div>
            ...
            <div class="form-row">
                <div class="form-group">
                    ...
                    <input type="color" id="primary_color" name="primary_color" 
                           value="<?= $_POST['primary_color'] ?? '#e8951c' ?>">
                </div>
                <div class="form-group">
                    <label for="secondary_color">Color Secundario:</label>
                    <input type="color" id="secondary_color" name="secondary_color" 
                           value="<?= $_POST['secondary_color'] ?? '#109c61' ?>">
                </div>
            </div>
            
            <div style="margin-top: 2rem;">
                <button type="submit" class="btn-submit">Crear Tarifa</button>
